...
整个匹配过程主要复杂的地方在于前缀匹配，在第三个步骤时，带修饰符和不带任何修饰符两者没有优先之分，都会倾向寻找最长的匹配，但如果这个最长匹配是由 `^~` 修饰的，则直接返回这个匹配。被不带修饰符的写法和带`^~`的写法的目标不能完全相同， Nginx 的配置检查将会无法通过，可以把一些更为详细的前缀匹配使用 `^~` 修饰，可以加快解析速度。
